...
public class BaseApiCall<T> implements Callback<T> {
    ...
    public BaseApiCall(BaseApi baseApi, CallId callId, CachePolicy cachePolicy, Callback<T> callback, Type responseType) {
        this.cachingDb = HeartBaymaxApplication.getApplication().getCachingDbHelper();
...
//            LogInternal.logBaseApiCall("answering valid-cache", callId.toString());
//            LogInternal.logBaseApiCall("skipping network call", callId.toString());
            success(cachedResponse.getObject(), null);
            return false;
        } else if (cachePolicy == CachePolicy.ANY_CACHE_THEN_NETWORK) {
//            LogInternal.logBaseApiCall("answering expired-cache", callId.toString());
            success(cachedResponse.getObject(), null);
